...
func TestSubstring(t *testing.T) {
	result := strutil.Substring("GoLang", 0, 2)
	expected := "Go"
	if result != expected {
...
func TestToCamelCase(t *testing.T) {
	result := strutil.ToCamelCase("go_lang_util")
	expected := "goLangUtil"
	if result != expected {
...
func TestToSnakeCase(t *testing.T) {
	result := strutil.ToSnakeCase("GoLangUtil")
...
func TestBase64(t *testing.T) {
	original := "hello"
	encoded := strutil.EncodeBase64(original)
	decoded, err := strutil.DecodeBase64(encoded)
...
func TestHasBlank(t *testing.T) {
	result := strutil.HasBlank("Go Lang")
	if !result {
...
func TestHasEmpty(t *testing.T) {
	result := strutil.HasEmpty("Go", "", "Lang")
	if !result {
...
func TestRemovePrefix(t *testing.T) {
	result := strutil.RemovePrefix("GoLang", "Go")
	expected := "Lang"
	if result != expected {
...
func TestRemoveSuffix(t *testing.T) {
	result := strutil.RemoveSuffix("GoLang", "Lang")
	expected := "Go"
	if result != expected {
...
func TestSub(t *testing.T) {
	result := strutil.Sub("GoLang", 0, 2)
	expected := "Go"
	if result != expected {
		t.Errorf("Sub() = %v; want %v", result, expected)
	}

	result = strutil.Sub("GoLang", -4, 4)
	expected = "Lang"
	if result != expected {
...
func TestStrBytes(t *testing.T) {
	str := "hello"
	bytes := strutil.Bytes(str)
	newStr := strutil.Str(bytes)
	if str != newStr {
...
func TestFormat(t *testing.T) {
	result := strutil.Format("Hello, %s!", "World")
